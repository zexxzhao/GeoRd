...
template <typename T, typename U,
          typename std::enable_if<
              std::is_integral<typename std::common_type<T, U>::type>::value,
              int>::type = 0>
...
void generate_distributed_tetrahedron_mesh(TetrahedronMesh &mesh) {
    ...
    auto n_owned_cells =
        std::count(element_belong_to.begin(), element_belong_to.end(), rank);
    ...
    mesh.vertex_local2global.assign(vertex_local_to_global.begin(),
                                    vertex_local_to_global.end());
}
    ...
    for (int i = 0; i < size; ++i) {
        if (i == rank) {
            ...
            for (int j = 0; j < mesh.vertices.size(); ++j) {
                std::cout << mesh.vertices[j][0] << " " << mesh.vertices[j][1]
                          << " " << mesh.vertices[j][2] << "\n";
            }
            std::cout << "Elements:\n";
            const auto l2g = mesh.vertex_local2global;
            for (int j = 0; j < mesh.elements.size(); j += 4) {
                std::cout << l2g[mesh.elements[j]] << " "
                          << l2g[mesh.elements[j + 1]] << " "
                          << l2g[mesh.elements[j + 2]] << " "
                          << l2g[mesh.elements[j + 3]] << "\n";
            }
            std::cout << "\n";
        }
