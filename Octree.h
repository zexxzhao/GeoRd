...
template <int D = typename Dim::value>
struct Node {
	Node(int n = is_empty){
		std::fill(vertex.begin(), vertex.end(), n);
	}

	void set_vertex(int i, int val) {
		this->vertex[i] = val;
	}
...
tempalte <int D = typename Dim::value>
struct TreeNode{
...
class Triangle3D{
	...
	Triangle3D(double xa, double ya, double za,
			 double xb, double yb, double zb,
			 double xc, double yc, double zc): Triangle3D(Point3D{xa,ya,za}, Point3D{xb,yb,zb}, Point3D{xc,yc,zc}) {}
	
	void print_tri(){
		std::cout << "Triangle3D Point 0: " << this->vertex[0].x() << " , " << this->vertex[0].y() << " , " << this->vertex[0].z() << std::endl;
		std::cout << "Triangle3D Point 1: " << this->vertex[1].x() << " , " << this->vertex[1].y() << " , " << this->vertex[1].z() << std::endl;
		std::cout << "Triangle3D Point 2: " << this->vertex[2].x() << " , " << this->vertex[2].y() << " , " << this->vertex[2].z() << std::endl;
	}
};
...
struct Box3D {
	...
	Box3D(double xa, double ya, double za,
			double xb, double yb, double zb) : Box3D(Point3D{xa, ya, za}, Point3D{xb, yb, zb}) {}
...
Box3D build_bounding_box(PointIT begin, PointIT end, double box_tol = 1e-6) {
	...
	auto xmax = std::max_element(begin, end, [](const Point3D &a, const Point3D &b) {
		return a[0] < b[0];
	});
	// get the maximum y coordinates of the point cloud
	auto ymax = std::max_element(begin, end, [](const Point3D &a, const Point3D &b) {
		return a[1] < b[1];
	});
	// get the maximum z coordinates of the point cloud
	auto zmax = std::max_element(begin, end, [](const Point3D &a, const Point3D &b) {
		return a[2] < b[2];
	});
	// get the minimum x coordinates of the point cloud
	auto xmin = std::min_element(begin, end, [](const Point3D &a, const Point3D &b) {
		return a[0] < b[0];
	});
	// get the minimum y coordinates of the point cloud
	auto ymin = std::min_element(begin, end, [](const Point3D &a, const Point3D &b) {
		return a[1] < b[1];
	});
	// get the minimum z coordinates of the point cloud
	auto zmin = std::min_element(begin, end, [](const Point3D &a, const Point3D &b) {
		return a[2] < b[2];
	});
...
struct Octree<Point3D, _Tag> {
	...
	void insert_point_help(Point pcur, int point_id, int level, Box3D box_cur, TreeNode& cur){
		...
		if( is_in_box==1 ){
			...
			for(int i=0;i<8;i++){
				...
				if( is_in_box == 1){
					cur.child_exist[i] = 1;
					insert_point_help(pcur,point_id,level+1,box_sub,cur.child[i]);
					break;
				}
			}
		}
	...
	void search_point_help(Point pcur, int level,
		...
		if( level == this->recur_lim ) {
			...
			for(int i=0;i<8;i++){
				if(cur.child_exist[i] == -1) continue;

				box_sub   = cal_sub_box(box_cur,i);
				double dist_tmp = compute_min_dist(box_sub,pcur);

				if(dist_tmp > rval) continue;
				search_point_help(pcur,level+1,cur.child[i],box_sub,index,rval);
			}
		}
	}
...
class Octree<Triangle3D, _Tag>{
	...
	void recur_insert(int tri_id, Box3D tri_box, int node_num, Box3D oct_box, int level, std::string path) {
		...
		for(int isub=0;isub<8;isub++){
			...
			if( node_dat[node_num].vertex[isub] == -1 and travel[isub] == true ){
				Node node_insert;
		...
		for(int isub=0;isub<8;isub++){
			if(travel[isub] == true){
				...
				recur_insert(tri_id, tri_box, node_dat[node_num].vertex[isub], box_sub[isub], level+1, path_next ); 
			}
		}
...
inline bool box_box_intersect(const Box3D &box1, const Box3D &box2){
	...
	wid_avg     = (box1.box_pos[1] - box1.box_pos[0])/2.0 + (box2.box_pos[1] - box2.box_pos[0])/2.0;
