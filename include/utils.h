...
template <typename T>
auto fp2str(T x) -> typename std::enable_if<std::is_floating_point<T>::value,
                                            std::string>::type {
...
template <typename T>
auto hash_triplet_impl(T x, T y, T z) ->
...
template <typename> std::false_type is_subscriptable_impl(...);
template <typename T>
auto is_subscriptable_impl(int)
...
template <typename T>
struct HashTable<T,
                 typename std::enable_if<Subscriptable<T>::value, void>::type> {
...
template <typename T>
auto key_equal_impl(const T &t1, const T &t2) ->
...
template <typename T>
struct KeyEqual<T, typename std::enable_if<Subscriptable<T>::value>::type> {
    bool operator()(const T &p1, const T &p2) const {
        return key_equal_impl(p1[0], p2[0]) && key_equal_impl(p1[1], p2[1]) &&
               key_equal_impl(p1[2], p2[2]);
    }
};
